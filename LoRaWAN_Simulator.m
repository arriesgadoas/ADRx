...
%choose what mode of simulation
%--option1: with classic ADR
%--option2: with FADR
%--option3: w/out adr
%--option0: use this mode for model development. creates annulus ring and
%take PDR
input_all_nodes_use_same_DR=0; 

%use inpur.csv
input_data = (readtable('input.csv'));
input_data = table2array(input_data);
number_of_runs = size(input_data,1);
config_output = zeros(size(input_data,1),2);

%edit this for single run simualtions
% if input_all_nodes_use_same_DR ~=0
%     number_of_runs = 1;
%     radius = 4;
%     power = 14;
%     coding_rate = 3;
...
for input_row = 1:1:number_of_runs
%     if input_all_nodes_use_same_DR  == 0
        fprintf('Run number %d of %d \n', input_row, size(input_data,1));
        radius = input_data(input_row,1);
        power = input_data(input_row,2);
        coding_rate = input_data(input_row,3);
        spreading_factor = input_data(input_row,4);
        number_of_nodes = input_data(input_row,5);
%     end 
   
config_num_iterations=10; %repeated measurement per run    
    
%FACTORS
config_loop_R=radius; %circular area radius in meters

input_UL_tx_power_dBm = [14 12 10 8 6 4 2]; %power levels: 14 12 10 8 6 4 2
switch power
    case 14
       input_fixed_pow=input_UL_tx_power_dBm(1);
    case 12
       input_fixed_pow=input_UL_tx_power_dBm(2);
    case 10
        input_fixed_pow=input_UL_tx_power_dBm(3);
    case 8
        input_fixed_pow=input_UL_tx_power_dBm(4);
    case 6
        input_fixed_pow=input_UL_tx_power_dBm(5);
    case 4
        input_fixed_pow=input_UL_tx_power_dBm(6);
    case 2
        input_fixed_pow=input_UL_tx_power_dBm(7);
end 


input_CR=coding_rate; %Coding rate = 4/CR+4;

input_DR=12-spreading_factor; %if all nodes use the same SF [12 11 10 9 8 7]

config_loop_EDs=number_of_nodes; %number of EDs

%SIMULATION PARAMETERS
input_simulation_duration_seconds=300; %duration of the simulated time window
input_draw_playground_map=true; %draw the "map" of the playground with GWs, EDs, and DR zones? true or false

input_energy_simulate=false; %simulate energy consumption
%config_num_iterations=10; %number of simulation iterations, each iteration may have different distributions of ED,GWs and UL packets (depends on the respective files)
...
input_DL_scheduling_probability=0;%probability the NS will generate a downlink in response to an UL
...
if input_all_nodes_use_same_DR==0
    input_func_ED_distribution=@km_LoRaWAN_model_ED_distribution;%controls ED placement in annulus rings 
else
    input_func_ED_distribution=@km_LoRaWAN_model_ED_distribution_circle;%controls ED placement 
end 
...
config_testcase_transmission=1; %case0 - multiple transmissions per ED, case1 - one transmission per ED
    ...
    for config_ctr_loop_DR_RX2=1:1:size(config_loop_DR_RX2,2)
        ...
        for config_ctr_loop_EDs=1:1:size(config_loop_EDs,2)
            ...
            for config_ctr_loop_GWs=1:1:size(config_loop_GWs,2)
                ...
                clearvars -except input_* config_* simulation input_fixed_pow
                config_seed=1234578;
...
%%%%%%%%%%%%%%%%%%%%%step 4: standard ADR - optimize the DRs and TX power of EDs in UL
                    ...
                    %match DR to SF
                    if input_all_nodes_use_same_DR==1
                      %  fprintf('Simulation using default ADR.\n');
                        for i=1:input_num_EDs
                        %UL and RX1
                            if(calc_DRs_UL(i)==0)
                                calc_SF_UL(i)=12;
                    ...
                    if (input_all_nodes_use_same_DR==0 || input_all_nodes_use_same_DR==3) 
                     %   fprintf('No ADR used. \n');
                       for i=1:input_num_EDs
                       ...
                       for i = 1:input_num_EDs
                           calc_UL_TX_power_matrix_dBm(i,1) = input_fixed_pow;
                       end
                    end
                    
                    
                    %FADR
                    if input_all_nodes_use_same_DR==2
                        [config_radius_per_cell,config_nodes_per_cell]=get_radius_and_nodes_vector(input_R,input_num_EDs,calc_distance_ED_GW_m);
                        [DR_vector,power_vector]=assign_sf_and_power(config_radius_per_cell,config_nodes_per_cell,calc_distance_ED_GW_m);
                        for i = 1:input_num_EDs
                           calc_UL_TX_power_matrix_dBm(i,1) = power_vector(i);
                           calc_DRs_UL(i) = DR_vector(i);
                           calc_DRs_RX2(i)=input_RX2_DR;
                        end
                        calc_DRs_RX1=calc_DRs_UL;
                    ...
                    if input_all_nodes_use_same_DR==4
                     start_index = 0;
                        for i=start_index+1:start_index+(floor(0.45*input_num_EDs))
                           calc_DRs_UL(i)=5; 
                           calc_DRs_RX2(i)=input_RX2_DR;
                           start_index = i;
                        end
                        for i=start_index+1:start_index+floor((0.27*input_num_EDs))
                           calc_DRs_UL(i)=4; 
                           calc_DRs_RX2(i)=input_RX2_DR;
                           start_index = i;
                        end
                        for i=start_index+1:start_index+floor((0.14*input_num_EDs))
                           calc_DRs_UL(i)=3; 
                           calc_DRs_RX2(i)=input_RX2_DR;
                           start_index = i;
                        end
                        for i=start_index+1:start_index+floor((0.08*input_num_EDs))
                           calc_DRs_UL(i)=2; 
                           calc_DRs_RX2(i)=input_RX2_DR;
                           start_index = i;
                        end
                        for i=start_index+1:start_index+floor((0.04*input_num_EDs))
                           calc_DRs_UL(i)=1; 
                           calc_DRs_RX2(i)=input_RX2_DR;
                           start_index = i;
                        end
                        for i=start_index+1:start_index+floor((0.02*input_num_EDs))
                           calc_DRs_UL(i)=0; 
                           calc_DRs_RX2(i)=input_RX2_DR;
                           start_index = i;
                        end
...
%remove DL interference     temp_DL_RX1_packet_flag_received=true;
                    ...
                    clearvars -except calc_* input_* result_* DEF_* config_* simulation input_fixed_pow
...
if input_draw_playground_map==1
   ...
   %GWs
   fig_scatter=scatter(calc_coord_GW_cartesian(:,1),calc_coord_GW_cartesian(:,2),'filled');
   fig_scatter.Marker='^';
   for temp_i_num_GW=1:input_num_GWs 
       txt = ['GW' num2str(temp_i_num_GW)];
       text(calc_coord_GW_cartesian(temp_i_num_GW,1)+15,calc_coord_GW_cartesian(temp_i_num_GW,2)+15,txt)
   end
   ylabel('Coordinate Y, m')
   xlabel('Coordinate X, m')
   title('ED and GW locations')
   hold on
   %EDs
   %fig_scatter=scatter(calc_coord_ED_cartesian(:,1),calc_coord_ED_cartesian(:,2));
   %fig_scatter.Marker='o';  
   legend_markers = ["GW"];
   for temp_i_num_ED=1:input_num_EDs 
       txt = [num2str(temp_i_num_ED)];
       if calc_DRs_UL(temp_i_num_ED)==0
           fig_scatter=scatter(calc_coord_ED_cartesian(temp_i_num_ED,1),calc_coord_ED_cartesian(temp_i_num_ED,2));
           fig_scatter.Marker='o';
           fig_scatter.MarkerFaceColor = [1 0 0]; %red
           fig_scatter.MarkerEdgeColor = [0 0 0];
           text(calc_coord_ED_cartesian(temp_i_num_ED,1)+15,calc_coord_ED_cartesian(temp_i_num_ED,2)+15,txt)
           if ismember("SF12", legend_markers)==0
               legend_markers(end+1)="SF12";    
           end
           hold on
       elseif calc_DRs_UL(temp_i_num_ED)==1
           fig_scatter=scatter(calc_coord_ED_cartesian(temp_i_num_ED,1),calc_coord_ED_cartesian(temp_i_num_ED,2));
           fig_scatter.Marker='o'; 
           fig_scatter.MarkerFaceColor = [1 0.65 0]; %orange
           fig_scatter.MarkerEdgeColor = [0 0 0];
           text(calc_coord_ED_cartesian(temp_i_num_ED,1)+15,calc_coord_ED_cartesian(temp_i_num_ED,2)+15,txt)
           if ismember("SF11", legend_markers)==0
               legend_markers(end+1)="SF11";
           end
           hold on
       elseif calc_DRs_UL(temp_i_num_ED)==2
           fig_scatter=scatter(calc_coord_ED_cartesian(temp_i_num_ED,1),calc_coord_ED_cartesian(temp_i_num_ED,2));
           fig_scatter.Marker='o';
           fig_scatter.MarkerFaceColor = [1 1 0]; %yellow
           fig_scatter.MarkerEdgeColor = [0 0 0];
           text(calc_coord_ED_cartesian(temp_i_num_ED,1)+15,calc_coord_ED_cartesian(temp_i_num_ED,2)+15,txt)
           if ismember("SF10", legend_markers)==0
               legend_markers(end+1)="SF10";
           end
           hold on
       elseif calc_DRs_UL(temp_i_num_ED)==3
           fig_scatter=scatter(calc_coord_ED_cartesian(temp_i_num_ED,1),calc_coord_ED_cartesian(temp_i_num_ED,2));
           fig_scatter.Marker='o';
           fig_scatter.MarkerFaceColor = [0 1 0]; %green
           fig_scatter.MarkerEdgeColor = [0 0 0];
           text(calc_coord_ED_cartesian(temp_i_num_ED,1)+15,calc_coord_ED_cartesian(temp_i_num_ED,2)+15,txt)
           if ismember("SF9", legend_markers)==0
               legend_markers(end+1)="SF9";
           end
           hold on
       elseif calc_DRs_UL(temp_i_num_ED)==4
           fig_scatter=scatter(calc_coord_ED_cartesian(temp_i_num_ED,1),calc_coord_ED_cartesian(temp_i_num_ED,2));
           fig_scatter.Marker='o';
           fig_scatter.MarkerFaceColor = [0 1 1]; %cyan
           fig_scatter.MarkerEdgeColor = [0 0 0];
           text(calc_coord_ED_cartesian(temp_i_num_ED,1)+15,calc_coord_ED_cartesian(temp_i_num_ED,2)+15,txt)
           if ismember("SF8", legend_markers)==0
               legend_markers(end+1)="SF8";
           end
           hold on
       else 
           fig_scatter=scatter(calc_coord_ED_cartesian(temp_i_num_ED,1),calc_coord_ED_cartesian(temp_i_num_ED,2));
           fig_scatter.Marker='o';
           fig_scatter.MarkerFaceColor = [0 0 1]; %blue
           fig_scatter.MarkerEdgeColor = [0 0 0];
           text(calc_coord_ED_cartesian(temp_i_num_ED,1)+15,calc_coord_ED_cartesian(temp_i_num_ED,2)+15,txt)
           if ismember("SF7", legend_markers)==0
               legend_markers(end+1)="SF7";
           end
           hold on
       end 
   end
...
sf_summary = [sf12 sf11 sf10 sf9 sf8 sf7];
%print results
config_output(input_row,1) = results_calc_all_iterations_UL_delivery_rate;


fprintf('Uplink Delivery Rate: %i\n', results_calc_all_iterations_UL_delivery_rate);

if input_energy_simulate == true
    fprintf('Total Energy Consumption: %i [mJ]\n', results_calc_all_iterations_energy_mean_total_consumption_mJ*input_num_EDs);
    config_output(input_row,2) = results_calc_all_iterations_energy_mean_total_consumption_mJ*input_num_EDs;

end 
end

output = array2table(config_output);
